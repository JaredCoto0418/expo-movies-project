...
export const topRatedMoviesPaginated = async (
  page: number = 1
): Promise<PaginatedMoviesResponse> => {
  try {
    const { data } = await moviesApi.get<MovieDBMoviesResponse>("/movie/top_rated", {
      params: {
        page,
      },
    });
    ...
    throw new Error('No se pudo obtener las películas mejor calificadas');
  }
};
